...
=head1 SYNOPSIS
  ...
  where <command> is one of login, get, mint, put, update, delete, fq, anvl, to_anvl

=head2 EXAMPLES

  # This Example gets a CDL in
  ezid get [--array] [--csv=I<columns>] [--header] ARK(s)
...
C<ezid login>: Allows users to set and save their login parameters

C<ezid mint>: Mint new Arks.

C<ezid update>: Updates an existing ARK

C<ezid delete>: Deletes ARKS if their I<_status:reserved>.
